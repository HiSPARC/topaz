...
#         plot_energy_profile_for_bin(seeds, gamma_e, electrons_e, muons_e)
        ...
        plot_energy_v_time_for_bin(seeds, gamma_t, electrons_t, muons_t,
...
#         plot.set_ylimits_for_all(None, 0, 120)
    ...
    plot.save_as_pdf('plots/time_profile_bin/%s.pdf' %
...
def plot_energy_profile_for_bin(seeds, gamma_e, electrons_e, muons_e):
    ...
    for splot, data, name in zip(plot.subplots,
                                 [gamma_e, electrons_e, muons_e],
                                 ['Gamma', 'Electrons', 'Muons']):
        splot.set_ylabel(name)
        splot.draw_vertical_line(300e6 if name is 'Muons' else 3e6, 'red')
        splot.histogram(*histogram(data, bins=bins))
...
def plot_energy_v_time_for_bin(seeds, gamma_t, electrons_t, muons_t,
                               gamma_e, electrons_e, muons_e):

    plot = MultiPlot(3, 1, 'loglog', height=r'.3\linewidth')
    e_bins = logspace(6, 12, 40)
    t_bins = logspace(log10(min(gamma_t.min(), electrons_t.min(), muons_t.min())),
                      log10(max(gamma_t.max(), electrons_t.max(), muons_t.max())),
                      25)
...
#         plot.set_ylimits_for_all(None, 0, 120)
    ...
    plot.subplots[-1].set_xlabel(r'Particle energy [\si{\MeV}]')
    plot.set_ylabel(r'Arrival time [\si{\ns}]')
    plot.save_as_pdf('plots/time_energy_2D/%s.pdf' %
                     get_info_string(seeds))
    ...
    with tables.open_file(PATH % seeds) as data:
        ...
        for splot, r_data, e_data, name in zip(plot.subplots,
            ...
            splot.histogram2d(counts, r_bins, e_bins, type='color',
                              colormap='viridis', bitmap=True)
            splot.draw_horizontal_line(300e6 if name is 'Muons' else 3e6, 'red')
...
def get_info_string_tex(seeds):
    info = get_info(seeds)
    return (r'%s, log10[E/\si{\eV}] = \SI{%.1f}, '
            r'$\theta$ = \SI{%.1f}{\degree}, I = \SI{%d}{\km}' %
            (name(info['particle_id']), log10(info['energy']),
             degrees(info['zenith']), info['first_interaction_altitude'] / 1e3))
