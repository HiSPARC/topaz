...
def download_data(station):
    ...
    with open_file(path, 'a') as data:
        ...
        esd.download_data(data, '/station_%d' % station, station, datetime(2015, 5, 28, 15), datetime(2015, 5, 29, 9))
        esd.download_data(data, '/station_%d' % station, station, datetime(2015, 6, 1, 8), datetime(2015, 6, 2, 11))

    path = os.path.join(DATA_PATH, 'muonlab_test2.h5')
    with open_file(path, 'a') as data:
        esd.download_data(data, '/station_%d' % station, station, datetime(2015, 6, 2, 12), datetime(2015, 6, 16))

    path = os.path.join(DATA_PATH, 'muonlab_test3.h5')
    with open_file(path, 'a') as data:
        esd.download_data(data, '/station_%d' % station, station, datetime(2015, 6, 2, 18), datetime(2015, 6, 25, 15))
