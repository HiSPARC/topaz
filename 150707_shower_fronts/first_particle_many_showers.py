...
import tables
from numpy import arange, save, load, sqrt, nan, isnan, histogram
...
def get_times():
    for e in arange(14, 18, 1):
        for core_distance in [1, 5, 10, 50, 100, 500, 1000]:
            tmp_times = [get_first_particle(seed, core_distance)
                         for seed in select_showers(e)]
            times = [t for t in tmp_times if not isnan(t)]
            path = ('/data/hisparc/adelaat/first_particle/first_{e}_{r}'
...
def plot_distribution():
    for e in arange(14, 18, 1):
        for core_distance in [1, 5, 10, 50, 100, 500, 1000]:
            ...
            plot.set_label('E=$10^{{{e}}}$eV, r={r}m'
                           .format(e=e, r=core_distance))
            plot.save_as_pdf('plots/first_particle_{e}_{r}_new'
                             .format(e=e, r=core_distance))
