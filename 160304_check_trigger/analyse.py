...
def determine_failed_percentrage(status):
    datapath = f'/Users/arne/Datastore/check_trigger/result_{status}_thresholds.h5'
    with tables.open_file(datapath, 'r') as data:
        t_trigger = data.root.s501.events.col('t_trigger')
        print(
            '%5.2f%% failed trigger reconstructions with %s thresholds'
            % (100.0 * sum(t_trigger == -999) / len(t_trigger), status)
        )
