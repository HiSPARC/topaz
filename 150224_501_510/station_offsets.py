...
from numpy import nan, isnan, arange, histogram, linspace, array, where
...
DETECTOR_OFFSETS = {501: [-1.064884, 0., 6.217017, 4.851398],
                    510: [9.416971, 0., 9.298256, 8.447724]}
...
def determine_station_timing_offsets(data):
    ...
    ref_t = array([where(ref_events.col('t1') == -999, 9000, ref_events.col('t1') - ref_d_off[0]),
    ...
    t = array([where(events.col('t1') == -999, 90000, events.col('t1') - d_off[0]),
    ...
    for event, ref_event in itertools.izip(events, ref_events):
        if (ref_event['t_trigger'] in ERR or event['t_trigger'] in ERR):
            dt.append(nan)
            continue
        dt.append((int(event['ext_timestamp']) - int(ref_event['ext_timestamp'])) -
