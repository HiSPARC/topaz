...
def reconstruct_simulations(data):
    ...
    for order in itertools.permutations(list(range(4)), 4):
        ...
        rec_501 = ReconstructESDEvents(data, station_group, station, overwrite=True, progress=True)
        ...
        high_zenith = (rec_501.theta > 0.2) & (rec_510.theta > 0.2)

        plot = Plot()
        plot.histogram2d(
            *histogram2d(
                rec_501.phi.compress(high_zenith), rec_510.phi.compress(high_zenith), bins=linspace(-pi, pi, 100)
            )
        )
