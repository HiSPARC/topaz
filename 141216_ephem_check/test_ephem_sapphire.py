...
def calc_astropy():
    ...
    location = EarthLocation(lon=Angle('%fd' % LONGITUDE), lat=Angle('%fd' % LATITUDE), height=ALTITUDE * u.m)
    altaz_frame = AltAz(obstime=obstime, location=location)
    altaz = SkyCoord('{:f}d {:f}d'.format(np.degrees(H_AZIMUTH), np.degrees(H_ALTITUDE)), frame=altaz_frame)
