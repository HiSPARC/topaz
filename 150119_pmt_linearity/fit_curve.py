...
def ice_cube_pmt(x, p0, p1, p2):
    ...
    """
    return x * exp(p0 * ((x / p1) ** p2) / ((1.0 - x / p1) ** 0.25))
...
def ice_cube_pmt_p1(x, p0, p2, p3):
    ...
    return x * exp(p0 * ((x / p3) ** p2) / ((1.0 - x / P1) ** 0.25))
...
def linear_errorfunction_linear(x, smoothness, slope_high, intercept_high):
    ...
    """
    slope_low, intercept_low = (1.0, 0.0)
    intercept = 10 ** ((intercept_high - intercept_low) / (slope_high - slope_low))
    erf_low = 1 - (erf((x - intercept) * smoothness) + 1) / 2.0
    erf_high = (erf((x - intercept) * smoothness) + 1) / 2.0
...
def lin_circ_lin(x, r, slope_low, intercept_low, slope_high, intercept_high):
    ...
    x_c = (parallel_intercept_high - parallel_intercept_low) / (slope_low - slope_high)
    ...
    perpendicular_intercept_low = x_c * (slope_low + 1 / slope_low) + parallel_intercept_low
    perpendicular_intercept_high = x_c * (slope_high + 1 / slope_high) + parallel_intercept_high

    # x positions where the lines transition to the circle and vice-versa
    intersect_circle_low_x = lin_intersection(
        slope_low, intercept_low, perpendicular_slope_low, perpendicular_intercept_low
    )
    intersect_circle_high_x = lin_intersection(
        slope_high, intercept_high, perpendicular_slope_high, perpendicular_intercept_high
    )

    y = where(x < intersect_circle_low_x, lin(x, slope_low, intercept_low), y_c - sign * sqrt(r**2 - (x - x_c) ** 2))
    y = where(x < intersect_circle_high_x, y, lin(x, slope_high, intercept_high))
...
def chisq(f, y, x, popt, s):
    ...
    """
    chisquare = sum(((yi - f(xi, *popt)) / si) ** 2 for yi, xi, si in zip(y, x, s))
...
def fit_curve(x, y, err=None, p0=(2.8, 2, -1)):
    ...
    """
    popt, pcov = curve_fit(fit_function, x, y, sigma=err, p0=p0, absolute_sigma=True)
    if all(popt == p0):
