...
def get_detector_locations(country=None, cluster=None, subcluster=None, station=None, stations=None):
    latitudes = []
    longitudes = []
    ...
    if station is not None:
        ...
        station_numbers = NETWORK.station_numbers(country=country, cluster=cluster, subcluster=subcluster)
...
def get_station_locations(country=None, cluster=None, subcluster=None, station=None, stations=None):
    latitudes = []
    longitudes = []
...
def make_map(
    ...
    if country is None and cluster is None and subcluster is None and station is None and stations is None:
        latitudes, longitudes = ([], [])
    else:
        latitudes, longitudes = get_locations(country, cluster, subcluster, station, stations)
    ...
    bounds = (
        min(latitudes + weather_latitudes + knmi_latitudes),
    ...
    if knmi:
        ...
        plot.scatter(x, map_h - y, mark='square', markstyle="mark size=0.5pt, black!50!blue, thick, opacity=0.6")
    ...
    if detectors:
        ...
        mark_size = 3
    plot.scatter(x, map_h - y, markstyle="mark size=%fpt, black!50!green, " "thick, opacity=0.9" % mark_size)
...
def main():
    ...
    parser.add_argument('--network', action='store_true', help='Map of the whole network')
    parser.add_argument('--country', action='store_true', help='Number represents the country')
    parser.add_argument('--cluster', action='store_true', help='Number represents the cluster')
    parser.add_argument('--subcluster', action='store_true', help='Number represents the subcluster')
    parser.add_argument('--station', action='store_true', help='Number represents the station')
    parser.add_argument('--stations', action='store_true', help='Number represents the station')
    parser.add_argument('--detectors', action='store_true', help='Show each detector')
    parser.add_argument('--weather', action='store_true', help='Show all HiSPARC weather stations')
    parser.add_argument('--knmi', action='store_true', help='Show all KNMI weather stations')
