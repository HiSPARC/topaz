...
def calculate_delta():
    ...
    tests = test_log_508()
    with tables.open_file(DATA_PATH, 'r') as data_file, tables.open_file(DELTAS_PATH, 'w') as delta_file:
        for test in tests:
            table = delta_file.create_table('/t%d' % test.id, 'delta', delta.DeltaVal, createparents=True)
...
def calculate(data, id):
    ...
    """
    coincidences, timestamps = search(data, ['/refr/t%d' % id, '/swap/t%d' % id], window=8000)
    deltas = []
    ext_timestamps = []
...
def plot_delta_test():
    ...
    plot.set_ylimits(min=0.0, max=0.15)
