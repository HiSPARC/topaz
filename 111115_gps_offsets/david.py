...
def calculate_delta():
    ...
    cable_length = 435.0
    tests = test_log_david()
    with tables.open_file(DATA_PATH, 'r') as data_file, tables.open_file(DELTA_PATH, 'w') as delta_file:
        for test in tests:
            table = delta_file.create_table('/t%d' % test.id, 'delta', DeltaVal, createparents=True)
