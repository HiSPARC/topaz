...
def plot_station_offset_matrix():
    n = len(STATIONS)
    stations = {station: Station(station, force_stale=True) for station in STATIONS}
    ...
    for type in ['offset', 'error']:
        ...
        plot.set_xlimits_for_all(None, min=datetime_to_gps(date(2011, 1, 1)), max=datetime_to_gps(date.today()))
...
def plot_cuts_vertically(splot, stations, station, ref_station):
    cuts = {
        ts
        for ts in chain(
            stations[station].gps_locations['timestamp'],
